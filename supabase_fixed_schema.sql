-- Fixed Supabase Schema for Airline App
...
-- Create journey_events table (if not exists)
...
-- Rename feedback to stage_feedback if needed
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'feedback' AND table_schema = 'public') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stage_feedback' AND table_schema = 'public') THEN
      ALTER TABLE feedback RENAME TO stage_feedback;
    END IF;
  END IF;
END $$;

-- Update stage_feedback table
ALTER TABLE stage_feedback ADD COLUMN IF NOT EXISTS journey_id UUID REFERENCES journeys(id) ON DELETE CASCADE;
ALTER TABLE stage_feedback ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES users(id) ON DELETE CASCADE;
