...
BEGIN
  ...
  BEGIN
    PERFORM net.http_post(
      url := supabase_url || '/functions/v1/flight-status-notification',
      ...
      body := jsonb_build_object(
        'journeyId', NEW.id,
        'userId', user_id_val,
        'oldStatus', COALESCE(OLD.status, ''),
        'newStatus', NEW.status,
        'oldPhase', COALESCE(OLD.current_phase, ''),
        'newPhase', COALESCE(NEW.current_phase, ''),
        'flightNumber', COALESCE(flight_info.flight_number, ''),
        'carrier', COALESCE(flight_info.carrier_code, ''),
