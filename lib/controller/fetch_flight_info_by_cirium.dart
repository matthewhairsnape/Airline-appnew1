  ...
  Future<Map<String, dynamic>> fetchFlightInfo({
    ...
    try {
      ...
      if (isHistorical) {
        ...
        url =
            'https://api.flightstats.com/flex/flightstatus/historical/rest/v3/json/flight/status/$cleanCarrier/$cleanFlightNumber/dep/'
            '$year/$month/$day'
        ...
        url =
            '$ciriumUrl/json/flight/status/$cleanCarrier/$cleanFlightNumber/dep/'
            '$year/$month/$day'
            '?appId=$ciriumAppId&appKey=$ciriumAppKey&airport=$departureAirport&extendedOptions=useHttpErrors';
      }

      debugPrint(
          '🌐 Cirium API URL (${isHistorical ? "HISTORICAL" : "REAL-TIME"}): $url');

      final response = await http.get(Uri.parse(url));

      debugPrint('📡 Cirium response status: ${response.statusCode}');
      debugPrint(
          '📦 Cirium response body (first 500 chars): ${response.body.substring(0, response.body.length > 500 ? 500 : response.body.length)}');

      if (response.statusCode == 200) {
        final Map<String, dynamic> data = json.decode(response.body);
        debugPrint(
            '✅ Cirium data parsed successfully. FlightStatuses count: ${data['flightStatuses']?.length ?? 0}');
        ...
        if (data['flightStatuses']?.isNotEmpty ?? false) {
          ...
          debugPrint(
              '   Route: ${flight['departureAirportFsCode']} → ${flight['arrivalAirportFsCode']}');
