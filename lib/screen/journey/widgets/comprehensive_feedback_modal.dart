...
class ComprehensiveFeedbackModal extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ComprehensiveFeedbackModal> createState() =>
      _ComprehensiveFeedbackModalState();
}

class _ComprehensiveFeedbackModalState extends ConsumerState<ComprehensiveFeedbackModal>
  ...
  Widget _buildCategorySection(
    ...
    return Column(
      ...
      children: [
          ...
          children: options.map((option) {
            ...
            return GestureDetector(
              onTap: () {
                ...
                // Just toggle selection - no navigation
        ...
        // Show media thumbnails immediately after upload (not just when comment box is open)
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.image, color: Colors.grey[700], size: 16),
                    SizedBox(width: 6),
                    Text(
                      'Uploaded Media',
                      style: AppStyles.textStyle_12_600.copyWith(color: Colors.grey[700]),
                    ),
                    Spacer(),
                    Text(
                      '${(isLikes ? _likesMediaFiles : _dislikesMediaFiles).length}',
                      style: AppStyles.textStyle_12_600.copyWith(color: Colors.grey[700]),
                    ),
                  ],
                ),
  ...
  Future<void> _submitFeedback() async {
    try {
      ...
      // Submit feedback for every phase so users can include Pre-Flight even after landing
      final phases = [
        {
          'phase': 'Pre-Flight',
  ...
  List<String> _getUnifiedLikes() {
    return _getPreFlightBubbleLikes();
  }

  List<String> _getUnifiedDislikes() {
    return _getPreFlightBubbleDislikes();
  }
  ...
  // Exact Pre-Flight bubble sets requested
  List<String> _getPreFlightBubbleLikes() {
    return [
      'Check-in process',
  ...
  List<String> _getPreFlightBubbleDislikes() {
    return [
      'Check-in process',
