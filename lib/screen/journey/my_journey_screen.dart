...
class _MyJourneyScreenState extends ConsumerState<MyJourneyScreen>
  ...
  */
    ...
    showModalBottomSheet(
      ...
      builder: (context) => ComprehensiveFeedbackModal(
        ...
        onSubmitted: () async {
          ...
          // Reload feedback status immediately and with retry to ensure real-time update
          if (flight.journeyId != null) {
            // First immediate check
            await _loadFeedbackStatusForFlight(flight.journeyId!);
            
            // Retry after delay to ensure database has processed
            await Future.delayed(Duration(milliseconds: 1000));
            await _loadFeedbackStatusForFlight(flight.journeyId!);
            
            debugPrint('✅ Feedback status refreshed after submission (including pre-flight)');
          }
          ...
          // Refresh the screen to update UI (including pre-flight button state)
          if (mounted) {
            setState(() {});
          }
        },
  ...
  Widget _buildPreFlightButton(FlightTrackingModel flight) {
    final flightId = flight.journeyId ?? flight.pnr ?? '';
    if (flightId.isEmpty) {
      return const SizedBox.shrink();
    }

    // Check if pre-flight feedback has been submitted
    final hasPreFlightFeedback = hasFeedbackForStage(flightId, 'pre_flight') || 
                                  hasFeedbackForStage(flightId, 'airport_review');

    return Container(
      decoration: BoxDecoration(
        color: hasPreFlightFeedback ? Colors.green[50] : Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: hasPreFlightFeedback ? Colors.green[300]! : Colors.grey[300]!,
          width: hasPreFlightFeedback ? 2 : 1,
        ),
        boxShadow: [
          BoxShadow(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: () {
            // Use the same review journey modal to keep design consistent
            _showReviewFlightModal(flight);
          },
          borderRadius: BorderRadius.circular(16),
          child: Padding(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: hasPreFlightFeedback 
                        ? Colors.green.withOpacity(0.1)
                        : Colors.blue.withOpacity(0.08),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Icon(
                    hasPreFlightFeedback ? Icons.check_circle : Icons.flight_takeoff,
                    color: hasPreFlightFeedback ? Colors.green : Colors.blue,
                    size: 22,
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        hasPreFlightFeedback ? 'Pre-Flight Reviewed' : 'Review Pre-Flight',
                        style: AppStyles.textStyle_16_600
                            .copyWith(color: Colors.black),
                      ),
                      const SizedBox(height: 4),
                      Text(
                        hasPreFlightFeedback 
                            ? 'Thank you for your feedback!'
                            : 'Share your airport experience before boarding',
                        style: AppStyles.textStyle_12_400
                            .copyWith(color: Colors.grey[700]),
                      ),
                    ],
                ...
                Icon(
                  hasPreFlightFeedback ? Icons.check : Icons.arrow_forward_ios_rounded,
                  color: hasPreFlightFeedback ? Colors.green : Colors.black54,
                  size: 18,
                ),
              ],
            ),
