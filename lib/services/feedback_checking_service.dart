...
class FeedbackCheckingService {
  ...
  static Future<Map<String, bool>> checkFeedbackStatus(String journeyId) async {
    try {
      ...
      // Check feedback table for overall experience and pre-flight
      final overallResponse = await _client
          .from('feedback')
          .select('id, phase')
          .eq('journey_id', journeyId)
          .or('phase.eq.landed,phase.eq.post-flight,phase.eq.arrival,phase.eq.overall,phase.eq.final,phase.eq.Pre-Flight,phase.eq.pre-flight,phase.eq.preflight')
          .limit(10);

      // Check for pre-flight in feedback table
      final preFlightInFeedback = overallResponse.any((item) {
        final phase = (item['phase'] as String? ?? '').toLowerCase();
        return phase.contains('pre') || phase.contains('pre-flight');
      });

      final feedbackStages = <String, bool>{
        'pre_flight': preFlightInFeedback || airportResponse.isNotEmpty, // Pre-flight can be in airport_reviews or feedback table
        'in_flight': false,
        'post_flight': false,
        'overall': overallResponse.any((item) {
          final phase = (item['phase'] as String? ?? '').toLowerCase();
          return phase.contains('post') || phase.contains('overall') || phase.contains('landed') || phase.contains('final');
        }),
