...
class FeedbackCheckingService {
  ...
  static Future<Map<String, dynamic>?> getExistingFeedback(String journeyId, String stage) async {
    try {
      debugPrint('🔍 Getting existing feedback for journey: $journeyId, stage: $stage');
  ...
  static Future<Map<String, Map<String, dynamic>?>> getAllFeedbackForJourney(String journeyId) async {
    try {
  ...
  static Future<int> getFeedbackCompletionPercentage(String journeyId) async {
    try {
      ...
      final completedStages = feedbackStatus.values.where((hasFeedback) => hasFeedback).length;
