...
class PhaseFeedbackService {
  ...
  static Future<bool> _submitAirportReview({
    ...
    try {
      ...
      // Also save to feedback table with phase 'Pre-Flight' for tracking
      try {
        await _client.from('feedback').upsert({
          ...
          'likes': _convertLikesToMap(likes),
          'dislikes': _convertDislikesToMap(dislikes),
          'comments': _createCommentFromSelections(likes, dislikes),
          if (mediaUrls.isNotEmpty) 'media_urls': mediaUrls,
          'created_at': DateTime.now().toIso8601String(),
          'updated_at': DateTime.now().toIso8601String(),
        });
        debugPrint('✅ Pre-Flight feedback also saved to feedback table');
      } catch (e) {
        debugPrint('⚠️ Could not save to feedback table (non-critical): $e');
      }
